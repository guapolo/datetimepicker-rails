class DatetimePickerInput < SimpleForm::Inputs::StringInput
  def input
    ...
    input_html_options[:class] << 'form-control date datetimepicker'
    template.content_tag :div, class: 'input-group' do
      input = super # leave StringInput do the real rendering
      input += template.content_tag :span, class: 'input-group-addon' do
        template.content_tag :span, '', class: 'glyphicon glyphicon-calendar', data: { 'time-icon' => 'glyphicon glyphicon-time', 'date-icon' => 'glyphicon glyphicon-calendar' }
      end
      input
    end
  end
